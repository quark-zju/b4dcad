...
面法线，不识别桥接或下方支撑。修形只处理沿 `+Z` 打印、没有洞、凸且
至少有一条连接边的水平悬垂区域；凹面、带洞面和所有边都连接的面保持
不变。斜面高度按点到最近连接边的距离计算，因此旋转和曲线边界也可处理。
`cut` 削除上方距离斜面，`add` 用距离斜面的补集向下加料，均不检查结果
是否断开或连接侧下方高度是否足够，并假定区域上下侧壁竖直、厚度不变。
